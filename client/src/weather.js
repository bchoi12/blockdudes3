...
export class Weather extends SceneComponent {
    constructor() {
        super();
        this._cloudLengths = [6, 6.5, 7, 7.5];
        this._cloudHeights = [0.6, 0.7, 0.8];
        this._cloudDepths = [4, 5, 6];
        this._cloudYs = [2, 4, 5, 6, 8, 10];
        this._cloudZs = [-12, -9, 6];
        this._cloudMaterials = new Array();
    ...
    newCloudMaterial(color, opacity) {
    ...
    newCloudMesh(x) {
        let cloud = new THREE.Mesh(new THREE.BoxGeometry(Util.randomElement(this._cloudLengths), Util.randomElement(this._cloudHeights), Util.randomElement(this._cloudDepths)), Util.randomElement(this._cloudMaterials));
        cloud.position.x = x;
